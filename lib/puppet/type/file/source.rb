...
module Puppet
  ...
  Puppet::Type.type(:file).newparam(:source) do
    ...
    cached_attr(:content) do
      ...
      unless tmp = Puppet::FileServing::Content.make_route( Puppet::Type::File.select_route( metadata.source ) ).find( metadata.source )
    ...
    cached_attr(:metadata) do
      ...
      value.each do |source|
        begin
          if data = Puppet::FileServing::Metadata.make_route( Puppet::Type::File.select_route(source) ).find(source)
            result = data
