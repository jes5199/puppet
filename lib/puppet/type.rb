...
class Type
    ...
    def self.namevar_parameter
        @namevar_parameter ||= (
            ...
            if params.length > 1
                raise Puppet::DevError, "Found multiple namevars for #{self.name}"
            elsif params.length == 1
                params.first
            else
                raise Puppet::DevError, "No namevar for #{self.name}"
            end
        )
    end
    ...
    def newattr(name, options = {})
        ...
        if provider and ! provider.class.supports_parameter?(klass)
            ...
            info "Provider #{provider.class.name} does not support features #{missing.join(", ")}; not managing attribute #{name}"
            return nil
        end
        ...
        if title = hash[:title]
            ...
            if self.namevar != :name
                if hash.include?(:name) and hash.include?(self.namevar)
                    raise Puppet::Error, "Cannot provide both name and #{self.namevar} to resources of type #{self.name}"
                end
            ...
            args.each { |property|
                ...
                unless propertyklass = @resource.class.validproperty?(property)
                    if @resource.class.validattr?(property)
                        ...
                        raise Puppet::Error, "#{property} is not a valid attribute for #{self.class.name}"
                    end
                end
        ...
        unless defined?(@schedule)
            if name = self[:schedule]
                if sched = catalog.resource(:schedule, name)
                    ...
                    self.fail "Could not find schedule #{name}"
                end
            else
                @schedule = nil
            end
    ...
    def scheduled?
        ...
        schedule.match?(self.cached(:checked).to_i)
    end
    ...
    def title
        unless @title
            ...
            if self.class.validparameter?(namevar)
                ...
                self.devfail "Could not find namevar #{namevar} for #{self.class.name}"
            end
        end
